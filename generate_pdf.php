<?php
    ...
    if(isset($_GET['gen_pdf']) && isset($_GET['id']))
    {
        ...
        $query = "SELECT bo.*, bd.*, uc.email FROM `booking_order` bo INNER JOIN `booking_details` bd ON bo.booking_id = bd.booking_id INNER JOIN `user_cred` uc ON bo.user_id = uc.id WHERE(bo.arrival=1 OR bo.refund=1) AND bo.booking_id = '$frm_data[id]'";
        ...
        // $mpdf->Output($data['name'].'-'.$data['room_name'].'.pdf','D');
